...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `id` FROM `vtiger_users` WHERE `user_name` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			//Creating password 
			//Fetching value of plain_pass and creating the salt (as in install.php)
			$__settings['plain_pass'] = $__settings['admin_pass'];
			 
			$__settings['salt'] = substr('admin', 0, 2);
			
			$__settings['salt'] = '$1$' . str_pad($__settings['salt'], 9, '0');
			
			// Do not change phpversion() to sphpversion()
			if(sversion_compare(phpversion(), '5.3.0', '<')){
				sconfigure('update_pass.php');	
				$__settings['admin_pass'] = swget($__settings['softurl'].'/update_pass.php');
				sunlink($__settings['softpath'].'/update_pass.php');
			}else{
				$__settings['admin_pass'] = crypt($__settings['plain_pass'], $__settings['salt']);
			}
			...
			$update_query = "UPDATE `vtiger_users` SET `user_password` = '".$__settings['admin_pass']."' WHERE `id` = '".$userid."';";
