...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		// This is to get dbprefix and salt from import.php
		...
		$query = "SELECT `id` FROM `".$__settings['dbprefix']."members` WHERE `login` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			$update_query = "UPDATE `".$__settings['dbprefix']."members` SET `password` = '".$__settings['admin_pass']."' WHERE `id` = '".$userid."';";
