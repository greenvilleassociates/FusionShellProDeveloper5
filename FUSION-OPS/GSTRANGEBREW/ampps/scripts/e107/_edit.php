...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `user_id` FROM `".$__settings['dbprefix']."user` WHERE `user_loginname` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			return false;			
				...
				// creating password using md5() function
				// We have hardcoded md5 for edit passwords since the same was used in 2.1.1 and previous versions as well. This is in CURL call from 2.1.1
				
				if(strlen($__settings['admin_pass']) < 8){
					$error[] = '{{err_passlen8}}';
				}
				
				$__settings['admin_pass'] = md5($__settings['admin_pass']);
			...
			$update_query = "UPDATE `".$__settings['dbprefix']."user` SET `user_password` = '".$__settings['admin_pass']."' WHERE `user_id` = '".$userid."';";
