...
global $__settings, $globals, $setupcontinue, $software, $error;
	...
	if(!empty($__settings['admin_pass'])){
		...
		$query = "SELECT `userID` FROM `".$__settings['dbprefix']."users` WHERE `name` = '".$__settings['admin_username']."';";
		...
		if(empty($userid)){
			...
			//Fetching password_salt from autoconf.php file
			$file = sfile($installation['softpath'].'/includes/autoconf.php');
			...
			soft_preg_replace('/\$password_salt(\s*?)=(\s*?)("|\')(.*?)("|\');/is', $file, $__settings['password_salt'], 4);
			...
			$update_query = "UPDATE `".$__settings['dbprefix']."users` SET `password` = '".$__settings['admin_pass']."' WHERE `userID` = '".$userid."';";
